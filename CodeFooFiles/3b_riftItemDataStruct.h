...
ItemObject
{
	...
	(ordered)map<currencyT, int> specialCurrencies; // currency-quantity pairs. Eg., special planarites, sourceshards, sourcestones
	
	(ordered)map<string, int> ingredients; // ingredient-quantity pairs. see [Note] at bottom
...
{	
	ItemObject item;
	vector<relationArc> related; /* each relationArc 1) specifies a type of relation and 2) points to another itemNode */
}


/*
 * [Note] I chose a collection of STRINGS for "ingredients" because there is such
